...
const NextMatches = ({id}) => {
  ...
  const dateFrom = `${year}-${month}-${day-1}`;
  const dateTo = `${year}-${month}-${Number(day) + 3}`;

  useEffect(() => {
    setLoading(true);
    axios.get(`/api/competitions/${id}/matches?dateFrom=${dateFrom}&dateTo=${dateTo}&`)
      .then(response => {
